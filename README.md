...
Локальные утилиты в одном репозитории:

| Путь | Приложение |
|---|---|
| корень (`client/`, `server/`) | **Docker Web Console** — управление контейнерами и образами |
| [`git-gantt/`](git-gantt/) | **Git Gantt** — временная диаграмма изменений git-репозитория |
...
Временная диаграмма изменений в git: коммиты по оси X, дерево папок и файлов по оси Y. Код и документация — в каталоге [`git-gantt/`](git-gantt/).
...
- Клиент: http://127.0.0.1:5317
- API: http://127.0.0.1:4317 (только localhost)
- Анализ подводных камней: [`git-gantt/docs/PITFALLS.md`](git-gantt/docs/PITFALLS.md)
- Архитектура: [`git-gantt/docs/ARCHITECTURE.md`](git-gantt/docs/ARCHITECTURE.md)
